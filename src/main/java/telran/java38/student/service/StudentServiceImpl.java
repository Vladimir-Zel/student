...
//				.collect(Collectors.toList());
		return studentRepository.findByGroup(group).map(s -> convertToStudentDto(s)).collect(Collectors.toList());
	}

	@Override
	public List<StudentDto> findStudentsByParameters(String group, String name) {
		return studentRepository.findByGroupAndName(group, name).map(s -> convertToStudentDto(s))
				.collect(Collectors.toList());
	}
